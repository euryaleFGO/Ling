...
class KnowledgeDAO:
    ...
    def save_character_settings(self, settings: Dict) -> bool:
        ...
        user_name = settings.get('user_name', '') or '用户'
...
用户称呼你为{settings.get('nickname', name)}，你称呼用户为{user_name}。
请用温柔、可爱的语气与用户对话。"""
        else:
            # 替换模板变量
        ...
        self.create_character(
            ...
            background="",
            system_prompt=system_prompt,
            greeting=f"你好呀，{user_name}~",
            extra=extra
        )
        
        self.set_active_character(name)
        
        # 同步用户称呼到 user_profiles 表
        if user_name:
            self.update_user_nickname(user_name=user_name)
        
        return True
    
    def update_user_nickname(self, user_id: str = "default_user", user_name: str = "") -> bool:
        """
        更新角色对用户的称呼
        ...
        if profile:
            return self.update_user_profile(user_id, {
                "nickname": user_name,
                "preferences.call_me": user_name
            })
        else:
            # 如果用户档案不存在，创建一个
            self.create_user_profile(user_id=user_id, nickname=user_name)
            return True
