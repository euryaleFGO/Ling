...
class CosyVoiceFrontEnd:
    ...
    def __init__(self,
        ...
        # CPU 提供程序配置（强制使用 CPU，适配 4GB 显卡）
        # 注意：Windows 上安装的 onnxruntime 通常是 CPU 版本，不支持 CUDA
        # 即使 torch.cuda.is_available() 返回 True，ONNX Runtime 的 CUDA 执行提供者也可能不可用
        cpu_providers = ["CPUExecutionProvider"]
        self.campplus_session = onnxruntime.InferenceSession(campplus_model, sess_options=option, providers=cpu_providers)
        
        # 强制使用 CPU 执行提供者（避免 CUDA 加载失败）
        # 对于 4GB 显卡，CPU 执行提供者也足够使用
        ...
        try:
            ...
            # 如果 CPU 执行提供者失败，尝试不指定提供者（让 ONNX Runtime 自动选择）
            logging.warning(f"使用 CPUExecutionProvider 失败，尝试自动选择: {e}")
