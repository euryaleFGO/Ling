...
class CosyvoiceRealTimeTTS:
    ...
    def _collect_audio_from_results(self, results):
        """将一次推理返回的多个 chunk 音频拼接为单段音频。"""
        audio_chunks = []
        for item in results:
            tts_speech = item.get('tts_speech') if isinstance(item, dict) else None
            if tts_speech is None:
                continue
            audio = tts_speech.squeeze().cpu().numpy().astype(np.float32)
            if audio.size == 0:
                continue
        ...
        merged_audio = audio_chunks[0] if len(audio_chunks) == 1 else np.concatenate(audio_chunks)
        max_abs = np.max(np.abs(merged_audio))
        if max_abs > 0:
            merged_audio = merged_audio / max_abs
        merged_audio = fade_in_out(merged_audio, self.sample_rate, self.fade_dur)
        return merged_audio
    ...
    def __init__(self, model_path: str, reference_audio_path: str = None, max_queue: int = 10, load_jit: bool = False, load_trt: bool = False):
        ...
        # TensorRT 需模型目录下存在对应 .plan 文件，否则自动关闭
        if load_trt:
            ...
            if not os.path.isfile(plan_fp16) and not os.path.isfile(plan_fp32):
                print(f"[WARN] 未找到 TensorRT plan 文件，TRT 已关闭")
                load_trt = False
        
        print(f"加载模型中... (JIT: {'启用' if load_jit else '禁用'}, TRT: {'启用' if load_trt else '禁用'}, FP16: 启用)")
        ...
        self.sample_text = "这是一段测试语音，喂喂喂你们听得到吗？让我看看啊别急。"
    ...
    def _synthesis_worker(self, segments, use_clone):
        for idx, seg in enumerate(segments, 1):
            ...
            try:
                ...
                # 3）拿音频（同一段的多个 chunk 进行拼接，避免丢后续内容）
                audio = self._collect_audio_from_results(results)
                if audio is None:
                    print(f"【合成】段 {idx} 未产出有效音频")
                    continue
    ...
    def generate_audio_streaming(self, text: str, use_clone=True, max_workers=None, spk_id: str = None):
        """
        ...
        Args:
            text: 要合成的文本
            use_clone: 是否使用语音克隆（当 spk_id 为 None 时有效）
            max_workers: 最大并行数（默认2，4GB显存）
            spk_id: 说话人 ID，若传入则使用已注册说话人而非克隆
        ...
        if spk_id:
            use_clone = False
        elif use_clone and self.ref_wav is None:
            use_clone = False
        ...
        def synthesis_worker(idx, seg):
            """合成单个段落"""
            if spk_id:
                result_idx, audio = self._generate_single_segment_with_speaker(idx, seg, spk_id)
            else:
...
if __name__ == "__main__":
    ...
    # 模型在 tts_server/models/TTS/CosyVoice2-0.5B
    project_root = Path(__file__).resolve().parent.parent
    model_path = project_root / "models" / "TTS" / "CosyVoice2-0.5B"
    ref_audio = project_root / "reference_audio" / "zjj.wav"
